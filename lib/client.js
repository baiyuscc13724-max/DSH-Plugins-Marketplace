        ...
        fetch("/api/marketplace/install", {
          ...
          if (data.status === "done" && data.installed) {
            setRepos(function (prev) {
              // 安装成功：更新卡片状态并立即按「已安装置顶」重排
              return installedFirstSort((prev || []).map(function (r) {
        ...
        repos ? (function () {
          ...
          return [
            ...
            list.map(function (repo) {
              ...
              return h(RepoCard, { key: repo.full_name, repo: repo, installed: repo.installed, updateAvailable: repo.updateAvailable, busy: !!(inst && inst.phase === "running"), selfBusy: !!(inst && inst.phase === "running" && inst.repo === repo.full_name), onInstall: function (fullName) { setInputValues({}); runInstall(fullName, {}, []); } });
            }),
