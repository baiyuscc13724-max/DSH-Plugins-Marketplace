...
async function runNpm(args, opts) {
  if (process.platform === "win32") {
    ...
    // Node 24 no longer launches Windows batch files directly through
    // child_process without a shell (it fails with spawn EINVAL). Invoke the
    // npm shim through cmd.exe explicitly so packaged Electron runtimes can
    // install dependencies using the npm available on PATH.
    const command = process.env.ComSpec || process.env.COMSPEC || "cmd.exe";
    return await execFileAsync(command, ["/d", "/s", "/c", "npm.cmd", ...args], opts);
  }
