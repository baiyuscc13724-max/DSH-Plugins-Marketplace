...
/**
 * npm install 回退链（安全默认：不执行第三方 install/prepare 脚本）：
 *   1) --ignore-scripts：默认不运行仓库的 prepare/pre/install/postinstall 脚本，
 *      使用仓库已提交的构建产物（lib/）——第三方代码不静默执行；
...
async function npmInstallWithFallback(cacheDir, env, logLine, lang) {
  const base = ["install", "--omit=dev", "--no-audit", "--no-fund", "--ignore-scripts"];
  const attempts = [
    { args: base },
...
export { apply, detectInstalled, loadOwnRepo, scanProfilePackages, langOf, t, fetchAllRepos, fetchRegistryRepos, isTrustedRequest, compareVersions, hasPatchEntry, normalizeRepo, appendPatchEntry };