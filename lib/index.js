...
/** 检测仓库是否已安装：优先查安装清单，其次按目录启发式探测（skill/预设/插件包/缓存克隆）。 */
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        for (const repo of repos) {
          ...
          if (!installedVersion) installedVersion = await readPackageVersion(join(PROFILE_NM, slug));
