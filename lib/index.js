...
/**
 * 从 profile 映射中按一组键查找匹配条目；带 repository 的条目必须与目标仓库一致，
 * 否则视为「同名撞仓库」（如别的作者也发布了叫 dsh-plugin-marketplace 的包），返回 null。
 */
function matchProfileEntry(profile, repo, keys) {
  ...
  for (const [dir, readPkg] of [[PROFILE_NM, true], [SKILLS_DIR, false], [PRESETS_DIR, false]]) {
    ...
    for (const entry of entries) {
      ...
      if (readPkg) {
        ...
        add(String(pkg.name ?? "").toLowerCase(), pkg.name ?? null, typeof pkg.version === "string" ? pkg.version : null, normalizeRepoRef(repoUrl));
      }
    }
  }
...
async function detectInstalled(repo) {
  ...
  // 包名映射（repository 校验防撞名：同名不同仓库的包不算已安装）
  if (matchProfileEntry(profile, repo, [slug, repo.name])) return true;
  ...
  if (pkgName && matchProfileEntry(profile, repo, [pkgName])) return true;
  return false;
}
        ...
        const worker = async () => {
          while (cursor < repos.length) {
            ...
            if (!installedVersion) {
              ...
              // 用包名映射表按仓库名/原始仓库名查找已装版本（repository 校验防撞名）。
              const hit = matchProfileEntry(profile, repo, [slug, repo.name]);
...
export { apply, detectInstalled, loadOwnRepo, scanProfilePackages, langOf, t, fetchAllRepos, fetchRegistryRepos, isTrustedRequest, compareVersions, hasPatchEntry, normalizeRepo, appendPatchEntry, readLifecycleScripts, sanitizeManifest, isPnpmLocalDependency, matchProfileEntry, normalizeRepoRef };