...
function apply(ctx) {
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        json(res, 200, { repos: flagged, cached_at: listCaches.dsh.at, total: flagged.length });
      } catch (error) {
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        json(res, 200, { repos: flagged, cached_at: listCaches.skills.at, total: flagged.length, filtered: list.length });
      } catch (error) {
        json(res, 500, { error: t(lang, "listFail", { err: String(error?.message ?? error) }) });
      }
    }
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      ...
      // m3：锁 key 归一化小写——Foo/Bar 与 foo/bar 的 cacheDir 相同（foo__bar），
      // 用大小写不同的写法并发安装时必须命中同一把锁，否则 rm/clone/cp 会互相踩踏。
      const lockKey = repo.toLowerCase();
      if (installLocks.has(lockKey)) return json(res, 409, { error: "install already in progress" });
      const task = (async () => {
        const log = [];
...
async function installRepo({ type, cacheDir, repo, log, answers, logLine, lang, envAllowList = [] }) {
  ...
  if (type === "cordis-plugin") {
    ...
    try {
      ...
      // 清洗 pnpm 专用本地链接依赖（link:/workspace:），否则 npm 解析 manifest 直接报 EUNSUPPORTEDPROTOCOL
...
export { apply, detectInstalled, detectSkillInstalled, loadOwnRepo, scanProfilePackages, langOf, t, fetchAllRepos, fetchRegistryRepos, isTrustedRequest, isTrustedHost, isSensitiveEnvKey, buildMinimalEnv, buildFilteredEnv, compareVersions, hasPatchEntry, normalizeRepo, appendPatchEntry, readLifecycleScripts, sanitizeManifest, isPnpmLocalDependency, matchProfileEntry, normalizeRepoRef, loadOfficialPackages, isOfficialPackage, readPackageSummary };