...
- **Full fetch**: the plugin list is served primarily from a **static registry** (`registry.json`, distributed via the jsDelivr CDN and regenerated daily by GitHub Actions) — zero API calls, zero rate limits, instant even with thousands of plugins; when the registry is unavailable it automatically falls back to paging the GitHub search API (sorted by star count descending, 10-minute cache)
...
```
GitHub Actions (daily 03:17 UTC, repo's own token)
   └─ scripts/build-registry.mjs: pages topic:dsh-plugin, incremental merge, dedupe/self-exclude
        └─ commits registry.json back to main (448+ plugins, sorted by stars)
...
- The registry is generated by CI, so end users make **zero API calls and hit no rate limits**; new plugins appear within a day at most
