...
- **搜索**：按插件名 / 仓库全名 / 标签实时过滤
- **通用 Skills 栏目**：设置页 tab 切换到「通用 Skills」——浏览 CI 构建的 skills 索引（`agent-skills` ∪ `claude-skills`，1800+ 仓库），支持搜索 / 分页触底加载 / 一键安装到 `~/.dsh/skills/` / 已安装识别；含安装脚本的仓库带 🛡 标识，探测未知的仓库带「未验证」弱提示
